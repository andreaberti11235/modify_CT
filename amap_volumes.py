...
dose_profiles = glob.glob('/Users/andreaberti/Desktop/tesi/activity_images/activity_images_RAI/activityMap_0_360_test_patient_*.nii.gz')
...
hist_values = hist_values/(dose_diff.shape[0] + 1)

tumor = nib.load('/Users/andreaberti/Desktop/tesi/pet_original_patient/tumor_volume.nii.gz')
...
for modified_PET in glob.glob(mpet_dir+'/*'):
    ...
    for i in range(dose_profiles.shape[1]):
        for j in range(dose_profiles.shape[2]):
            for k in range(dose_profiles.shape[3]):
                ...
                p_values[i, j, k] += 1/(dose_diff.shape[0] + 1)
    ...
    voxels = np.count_nonzero(p_values_negatives_reverse[tumor.get_fdata()>0])
