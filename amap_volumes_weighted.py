...
# use the most extreme case to create a mask of the tumor in the IDM
...
original_PET = '/Users/andreaberti/Desktop/tesi/activity_images/activityMap_0_360_test_patient_5.nii.gz'
original_PET = np.array(nib.load(original_PET).get_fdata())
p_values[p_values>threshold] = 1
p_values_negatives = p_values.copy()
p_values_negatives[modified_sign!=-1] = 1
mask = 1-p_values_negatives
mask[:59, :, :] = 0
mask[84:, :, :] = 0
mask[:, :30, :] = 0
mask[:, 48:, :] = 0
mask[:, :, :75] = 0
mask[:, :, 95:] = 0
gray_mean = np.sum(original_PET[mask!=0])
gray_mean /= np.count_nonzero(mask)
...
for modified_PET in glob.glob(mpet_dir+'/*'):
    ...
    #voxels = np.count_nonzero(p_values_negatives_reverse[:,30:48,75:95])
    voxels = np.count_nonzero(p_values_negatives_reverse[59:84,30:48,75:95])
    p_values_negatives_reverse[:59, :, :] = 0
    p_values_negatives_reverse[84:, :, :] = 0
    p_values_negatives_reverse[:, :30, :] = 0
    p_values_negatives_reverse[:, 48:, :] = 0
    p_values_negatives_reverse[:, :, :75] = 0
    p_values_negatives_reverse[:, :, 95:] = 0
    gray_sum = np.sum(original_PET[p_values_negatives_reverse!=0])
    weighted_volume = 1.6 * 1.6 * 1.6 * gray_sum / gray_mean
    volume = voxels * 1.6 * 1.6 * 1.6
    print('{},{}'.format(name,weighted_volume))